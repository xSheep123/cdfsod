...
def main(model='vitl14', dataset='clipart1k_1shot', use_bbox='yes',
    ...
    with tqdm(total=epochs * len(dataloader)) as bar:
        for _ in range(epochs):
            for item_i, item in enumerate(dataloader):
                ...
                if ('generate' in filename) and (int(filename.split("_")[3]) != 0):
                    continue
                ...
                if 'stuff' in dataset_name:
                    ...
                    for bmask, label, bbox in zip(masks_used, instances.gt_classes, 
                        ...
                        if 'generate' in os.path.basename(item['file_name']):
                            dataset['generate'].append(True)
                        else:
                            dataset['generate'].append(False)
