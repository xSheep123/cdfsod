...
def main(inp, 
    ...
    if len(dataset['avg_patch_tokens']) > 0:
        ...
        generate = torch.as_tensor(dataset['generate']) #是否是生成图片对应的token
        ...
        assert len(tokens) == len(labels) == len(generate)
    else:
        # background tokens
    ...
    tdataset = torch.cat([tokens, labels[:, None], generate[:, None]], dim=1)
    ...
    with tqdm(total=epochs * len(dloader), desc='learning prototypes') as bar:
        for _ in range(epochs):
            for batch in dloader:
                ...
                batch_tokens = batch[:, :-2]
                batch_labels = batch[:, -2]
                batch_generates = batch[:, -1]
                prototypes.update(tokens=batch_tokens, labels=batch_labels, generates=batch_generates)
                bar.update()
