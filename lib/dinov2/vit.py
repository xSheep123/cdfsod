...
class DinoVisionTransformer(Backbone):
    ...
    def forward(self, x, is_training=False, return_class_token=False, **kwargs):
        # from pudb.remote import set_trace; set_trace(term_size=(202, 47), port=12345)  
        self.enforce_partial_freeze_if_needed()
        outputs = self.get_intermediate_layers(x, n=self._out_indices, reshape=True, return_class_token=return_class_token)
        if return_class_token:
