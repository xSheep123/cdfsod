...
class PrototypeLearner(nn.Module):
    def __init__(
        ...
        self.local_queue = torch.zeros(queue_size, embed_dim + 2)
    ...
    def update(self, tokens, labels, generates, delay=False):
        """
        tokens: [N, embed_dim]
        labels: [N]
        """
        ...
        for c in torch.unique(labels):
            ...
            class2data[int(c.item())] = torch.cat(
                [tokens[mask], labels[mask, None], generates[mask, None]], dim=1
            )
        ...
        for r in range(self.world_size):
            ...
            rank2data[r] = (
                torch.cat(_)
                if len(_) > 0
                else torch.zeros(0, self.embed_dim + 2, device=device)
            )
        ...
        if len(recv_data) > 0:
            ...
            tokens, labels, generates = (
                self.local_queue[: self.local_queue_size, :-2],
                self.local_queue[: self.local_queue_size, -2],
            ...
            # generates[generates == True] = self.weight  # 生成类型获得指定权重
            # generates[generates == False] = 1   # 原始特征保持
            # expand = generates.repeat(pi.shape[0], 1) # 给定生成类原型一定权重
            # pi = pi * expand
