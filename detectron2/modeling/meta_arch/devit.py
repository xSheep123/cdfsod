...
def spearman_loss(cos_sim1, cos_sim2):
    """ 计算 Spearman 排序一致性损失 """
    # 获取排名索引
    ranks1 = torch.argsort(torch.argsort(cos_sim1, dim=-1), dim=-1)
    ranks2 = torch.argsort(torch.argsort(cos_sim2, dim=-1), dim=-1)
    
    # 计算排名的 MSE 损失
    return torch.mean((ranks1 - ranks2).float() ** 2)
...
def normalize_boxes_list( boxes_list, widths, heights):
            ''''
            坐标归一化
            '''
            normalized_list = []
            for boxes, w, h in zip(boxes_list, widths, heights):
                normalized_boxes = boxes.clone().float()
                normalized_boxes[:, [0, 2]] /= w  
                normalized_boxes[:, [1, 3]] /= h  
                normalized_list.append(normalized_boxes)
...
def _log_classification_stats(pred_logits, gt_classes):
    ...
    bg_inds = gt_classes == bg_class_ind  # 真实背景样本索引
    num_bg = bg_inds.nonzero().numel()  # 背景样本总数

    bg_pred_classes = pred_classes[bg_inds]  # 预测类别
    num_false_positive = (bg_pred_classes != bg_class_ind).nonzero().numel()  # 背景误识别为前景的数量

    try:
        storage = get_event_storage()
        storage.put_scalar(f"cls_acc", num_accurate / num_instances)
        if num_fg > 0:
            storage.put_scalar(f"fg_cls_acc", fg_num_accurate / num_fg)  #前景中正确的类别
            storage.put_scalar(f"false_neg_ratio", num_false_negative / num_fg)

        if num_bg > 0:
            storage.put_scalar(f"false_pos_ratio", num_false_positive / num_bg)  # 背景误识别为前景的比例
    except:
        pass
...
def js_divergence(p_logits, q_logits):
    """计算 Jensen-Shannon 散度，输入为 logits 需先 softmax"""
    p = F.softmax(p_logits, dim=-1)  # 预测分布
    q = F.softmax(q_logits, dim=-1)  # 真实分布（同样进行 softmax）

    m = 0.5 * (p + q)  # 计算混合分布
    jsd = 0.5 * F.kl_div(p.log(), m, reduction='mean') + \
          0.5 * F.kl_div(q.log(), m, reduction='mean')
    return jsd
...
def mask(predictor, img):
    padding = 50
    img_pad = F.pad(img, (padding, padding, padding, padding), mode='constant', value=0)
    img_pad = img_pad.permute(1, 2, 0).cpu().numpy()
    predictor.set_image(img_pad)
    input_box = np.array([padding, padding, img.shape[-1]+padding, img.shape[-2]+padding])
    masks, scores, logits = predictor.predict(
                    box=input_box,
                    multimask_output=False  # 设置为 False 只输出一个掩码
                    )
    mask = masks[0]
    mask = mask[padding:padding+img.shape[-2],padding:padding+img.shape[-1]]
    mask = torch.tensor(mask, dtype=torch.bool, device=img.device)
    img[:, ~mask] = 255
...
class InstReweight(nn.Module):
    ...
    def forward(self, x):
        ...
        out_normal = F.normalize(out, dim=-1)
        return out_normal, attn_score, out
...
class OpenSetDetectorWithExamples(nn.Module):
    ...
    def __init__(self,
        ...
        if len(prototypes.shape) == 3:
            if self.set_cd_vito:
                ...
                self.pro_weight = class_weight
                self.ema_geo_loss_cls = None
    ...
    @classmethod
    def from_config(cls, cfg, flag=1,use_bn=False):
        offline_cfg = get_cfg()
        ...
        return {
            ...
            "class_prototypes_file": class_prototypes_file,
            ...
            "geo_weight": cfg.MODEL.GEO_WEIGHT,
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]], 
        ...
        if self.set_cd_vito:
            self.train_class_weight, _, prototypes_no_normal = self.adapter(self.class_weights.weight.view(-1, self.k_shot, self.ndim))
        ...
        with torch.no_grad():
            ...
            #     print(os.path.basename(batched_inputs[idx]['file_name']))
                
            if inputs_1 is not None:
                images1 = self.preprocess_image(inputs_1)
                images2 = self.preprocess_image(inputs_2)
        ...
        with torch.no_grad():
            ...
            with autocast(enabled=True):
                ...
                if inputs_1 is not None:
                    patch_tokens_1 = self.backbone(images1.tensor)[self.vit_feat_name]
                    patch_tokens_2 = self.backbone(images2.tensor)[self.vit_feat_name]
        ...
        if inputs_1 is not None:
            ...
            box1 = []
            box2 = []
            for idx in range(len(inputs_1)):
                name = os.path.basename(inputs_2[idx]['file_name'])
                ann_idx = int(name.split("_")[-1].split(".")[0])  #对应第几个标注
                if 'generate' in os.path.basename(inputs_1[idx]['file_name']):
                    ann_idx = 0

                assert inputs_1[idx]['instances'][ann_idx].gt_classes == inputs_2[idx]['instances'][0].gt_classes
                #target.append(inputs_2[idx]['instances'][0].gt_classes)
                
                box1.append(inputs_1[idx]['instances'].gt_boxes.tensor[ann_idx])
                box2.append(inputs_2[idx]['instances'].gt_boxes.tensor[0])

            #target = torch.tensor(target).to(self.device)
            box1 = torch.stack(box1).to(self.device)
            box2 = torch.stack(box2).to(self.device)
            bid = torch.arange(len(inputs_1)).unsqueeze(1).to(self.device)
            rois1 = torch.cat((bid, box1), dim=1)
            rois2 = torch.cat((bid, box2), dim=1)

            ins_feats1 = self.roi_align(patch_tokens_1, rois1)
            ins_feats2 = self.roi_align(patch_tokens_2, rois2)

            ins_feats1 = F.normalize(ins_feats1.flatten(2).mean(2), dim=1)
            ins_feats2 = F.normalize(ins_feats2.flatten(2).mean(2), dim=1)

            cos_sim1 = ins_feats1 @ class_weights.T
            cos_sim2 = ins_feats2 @ class_weights.T
           
           
            if num_classes == 1:
                warmup_factor = min(1.0, iter / 100)
                loss_dict['geo_loss_cls'] =warmup_factor * self.geo_weight * F.mse_loss(cos_sim2, cos_sim1)
            else:
                warmup_factor = 1
                if num_classes == 3:
                    warmup_factor = min(1.0, iter / 30)
                loss_dict['geo_loss_cls'] = warmup_factor * self.geo_weight * F.kl_div(cos_sim2.log_softmax(dim=-1), cos_sim1.softmax(dim=-1),reduction='batchmean')
        ...
        if self.training or self.use_one_shot:
            with torch.no_grad():
                ...
                for idx , (proposals_per_image, targets_per_image) in enumerate(zip(boxes, gt_instances)):
                    ...
                    matched_gt_id_i = torch.full((class_labels_i.shape[0],), idx).to(self.device)                    
                    ...
                    matched_gt_id.append(matched_gt_id_i)
                    if self.use_mask:
                ...
                matched_gt_id = torch.cat(matched_gt_id)# for regression purpose.
                if self.use_mask:
        ...
        if (self.training and (not self.only_train_mask)) or (not self.training):
            ...
            if self.training:
                ...
                matched_gt_id = matched_gt_id[fg_indices]
        ...
        if self.training:
            ...
            if not self.only_train_mask:
                gt_pred_deltas = self.box2box_transform.get_deltas(
                    fg_proposals,  #前景候选框和gt的变化量
                    matched_gt_boxes,
                )
                loss_box_reg = smooth_l1_loss(  # 预测的变化量
