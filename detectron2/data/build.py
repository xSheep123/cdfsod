...
def _train_loader_from_config(cfg, mapper=None, *, dataset=None, sampler=None):
    if 'imgtxtpairs' in cfg.DATASETS.TRAIN:  # dataset, transform/aug., sampler for image-text pairs training
        ...
        return {
            ...
            "offset": cfg.DATASETS.NUM_ANNOTATIONS
        }
    ...
    if sampler is None:
        sampler_name = cfg.DATALOADER.SAMPLER_TRAIN
        num_anno = cfg.DATASETS.NUM_ANNOTATIONS
        num_img = cfg.DATASETS.NUM_IMG
        logger = logging.getLogger(__name__)
        logger.info("Using training sampler {}".format(sampler_name))
        if sampler_name == "TrainingSampler":
            sampler = TrainingSampler(len(dataset), num_anno, num_img)
        elif sampler_name == "RepeatFactorTrainingSampler":
            repeat_factors = RepeatFactorTrainingSampler.repeat_factors_from_category_frequency(
                dataset, cfg.DATALOADER.REPEAT_THRESHOLD
            )
...
def build_detection_train_loader(
    dataset, *, mapper, sampler=None, total_batch_size, aspect_ratio_grouping=True, num_workers=0, offset=0, start=0
):
    """
    ...
    if mapper is not None:
        dataset = MapDataset(dataset, mapper, offset=offset, start_idx=start) 
    if sampler is None:
        sampler = TrainingSampler(len(dataset))
...
def build_detection_test_loader(dataset, *, mapper, sampler=None, num_workers=0, clip_batch_size=None):
    ...
    if isinstance(dataset, list):
        dataset = DatasetFromList(dataset, copy=False)
    if mapper is not None:
        dataset = MapDataset(dataset, mapper, offset=0, start_idx=0,training=False)
    if sampler is None:
